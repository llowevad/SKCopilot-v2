// ASP.NET JWT Bearer authentication for Bot Framework Service and Entra ID tokens.
// Adapted from the official Microsoft M365 Agents SDK sample:
// https://github.com/microsoft/Agents-for-net/blob/main/src/samples/A2AAgent/AspNetExtensions.cs
...
public static class AspNetExtensions
{
    ...
    /// <summary>
    /// Adds ASP.NET token validation for Azure Bot Service and Entra ID using the "TokenValidation" config section.
    /// </summary>
    public static void AddAgentAspNetAuthentication(this IServiceCollection services, IConfiguration configuration, string tokenValidationSectionName = "TokenValidation")
    {
        ...
        {
            System.Diagnostics.Trace.WriteLine("AddAgentAspNetAuthentication: Auth disabled — TokenValidation section missing or Enabled=false");
            return;
        }
    ...
    /// <summary>
    /// Adds ASP.NET token validation for Azure Bot Service and Entra ID.
    /// </summary>
    public static void AddAgentAspNetAuthentication(this IServiceCollection services, TokenValidationOptions validationOptions)
    {
        ...
        // Filter to GUID-only audiences for JWT validation (non-GUID values like URLs are used by CloudAdapter internally)
        var guidAudiences = validationOptions.Audiences.Where(a => Guid.TryParse(a, out _)).ToList();
        if (guidAudiences.Count == 0)
        {
            throw new ArgumentException($"{nameof(TokenValidationOptions)}:Audiences requires at least one GUID-format ClientId");
        }

        // Default valid issuers for ABS Public Cloud
            ...
            options.Events = new JwtBearerEvents
            {
                ...
                OnTokenValidated = context =>
                {
                    ...
                    logger.LogInformation(
                        "🟢 AUTH OK for {Method} {Path} — Identity: {Identity}, Issuer: {Issuer}",
                        context.Request.Method,
                        context.Request.Path,
                        identity?.Name ?? "(anonymous)",
                        context.Principal?.Claims.FirstOrDefault(c => c.Type == "iss")?.Value ?? "(unknown)");
                ...
                OnAuthenticationFailed = context =>
                {
                    var logger = context.HttpContext.RequestServices.GetRequiredService<ILoggerFactory>().CreateLogger("JwtAuth");
                    logger.LogWarning(
                        "🔴 AUTH FAILED for {Method} {Path} — {Error}",
                        context.Request.Method,
                        context.Request.Path,
